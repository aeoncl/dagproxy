...
use windows::Win32::NetworkManagement::IpHelper::{GetAdapterIndex, GetAdaptersAddresses, GAA_FLAG_INCLUDE_GATEWAYS, GAA_FLAG_SKIP_ANYCAST, GAA_FLAG_SKIP_MULTICAST, IP_ADAPTER_UNICAST_ADDRESS_LH};
...
pub(crate) fn list_interfaces() -> Result<List, Error> {
    ...
    unsafe {
        ...
        while !adapter_ptr.is_null() {
            ...
            // On Windows, IfIndex 0 is reserved and means "No interface".
            // Valid interfaces are always 1 or greater.
            let ipv4_index =  adapter.Anonymous1.Anonymous.IfIndex;
            let ipv6_index = adapter.Ipv6IfIndex;
            let ifindex = if ipv6_index != 0 {
                ipv6_index
            } else if ipv4_index != 0 {
                ipv4_index
            } else {
                // If both are 0, this is a phantom or uninitialized adapter.
                // We must skip it to avoid HashMap key collisions.
                adapter_ptr = adapter.Next;
                continue;
            };
